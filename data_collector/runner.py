...
def insert_think(gen_output: GeneratorOutput):
    reasoning_key = ["reasoning_content", "reasoning"]
    if gen_output.raw_response is not None:
        raw_response = json.loads(gen_output.raw_response)
        for reasoning_key_i in reasoning_key:
            if reasoning_key_i in raw_response['choices'][0]["message"]:
                if raw_response['choices'][0]["message"][reasoning_key_i] is not None:
                    gen_output.output = "<think>\n" + raw_response['choices'][0]["message"][reasoning_key_i] + "\n</think>\n" + raw_response['choices'][0]["message"]['content']
                    return gen_output
    return gen_output
    ...
    def _process_records_concurrent(self, data: List[Dict[str, Any]], generator, evaluator, concurrency: int, model_config=None) -> List[RecordResult]:
        ...
        def process_single_record(record_data: Dict[str, Any], index: int) -> RecordResult:
            ...
            try:
                ...
                gen_output = insert_think(gen_output)
